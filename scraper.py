...
async def get_interesting_fact():
    facts = [
        "🏎️ **Факт:** Самый быстрый пит-стоп в истории F1 — 1.82 секунды (Red Bull, 2019)",
        "🔧 **Факт:** Шина F1 весит около 10 кг, а давление в ней — до 25 PSI",
        "⚡ **Факт:** MGU-K генерирует до 120 кВт (163 л.с.)",
        "🏆 **Факт:** Михаэль Шумахер — единственный, кто выигрывал чемпионат 5 раз подряд",
        "🔥 **Факт:** Самый молодой чемпион — Себастьян Феттель (23 года 134 дня)"
    ]
...
async def generate_morning_digest():
    ...
    fact = await get_interesting_fact()
    ...
    digest += f"{fact}\n\n"
    digest += f"{quote}\n\n"
    digest += f"{weather[:200]}\n\n"
    digest += f"<i>Хорошего дня! 🏁</i>\n\n"
    digest += f"<code>Nico 6.5 | Code by: RedRace Development</code>"
