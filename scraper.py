...
if GEMINI_KEY:
    ...
    print("✅ Gemini 1.5 Flash подключен")
else:
    gemini_model = None
...
async def ask_hunter(prompt: str) -> str:
...
async def ask_healer(prompt: str, image_url: str = None) -> str:
    if not openrouter_client:
        return await ask_gemini(prompt)
    try:
        messages = [{"role": "user", "content": prompt}]
...
async def random_post() -> str:
    ...
    return clean_post(await ask_hunter(prompt)) + "\n\nRed Race | Подписаться"
...
# === МОНИТОРИНГ RSS (РУЧНАЯ ПУБЛИКАЦИЯ) ===
pending_posts = []
