...
Разбивай на 3-4 абзаца. Каждый абзац — 2-3 предложения."""
    post = await ask_llm(prompt)
    post = clean_post(post)
    return post + "\n\nRed Race | Подписаться"
...
async def chat_reply(msg: str, is_group=False) -> str:
    context = ""
    if is_group:
        context = "Ты в групповом чате. Отвечай кратко, по делу. Не используй эмодзи в каждом сообщении.\n\n"
    prompt = f"{context}Ты Нико, эксперт по Формуле-1. Ответь кратко, по делу.\n\nВопрос: {msg}"
    return await ask_llm(prompt)
