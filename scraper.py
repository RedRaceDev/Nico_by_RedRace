...
async def chat_reply(user_id: int, message: str, photo_url: str = None) -> str:
    # 1. ЖЁСТКИЙ ПЕРЕХВАТ ВОПРОСОВ ПРО ЧЕМПИОНАТ
    if any(word in message.lower() for word in ['чемпионат', 'лидирует', 'очки', 'таблица', 'лидер', 'standings', 'championship']):
        standings = await get_driver_standings(2026)
        if standings:
    ...
    # 3. ТЕХНИЧЕСКИЕ ВОПРОСЫ — ЧЕСТНО ГОВОРИМ "НЕТ ДАННЫХ"
