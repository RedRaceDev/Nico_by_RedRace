...
FREE_MODELS = [
    "x-ai/mimo-v2-pro:free",           # Xiaomi MiMo-V2-Pro
...
def is_fresh_news(entry) -> bool:
    """Проверяет, что новость не старше 7 дней"""
    published = entry.get('published_parsed')
    if not published:
        title = entry.get('title', '')
        old_years = ['2024', '2025', '2023', '2022', '2021', '2020']
        for year in old_years:
            if year in title:
                print(f"📅 Пропуск: старый год {year} в заголовке")
                return False
        return True
    ...
    if days_ago > 7:
        print(f"📅 Пропуск: новость от {pub_date.strftime('%d.%m.%Y')} (>{days_ago} дней)")
        return False
    
    return True

def has_old_date(text: str) -> bool:
    """Проверяет, упоминается ли в посте старый год"""
...
def extract_article_sync(url: str) -> dict:
    try:
        ...
        return {"title": "", "text": "", "summary": "", "top_image": None, "publish_date": None}
...
async def gen_post(title: str, content: str, source_url: str) -> str:
    ...
    prompt = f"""Ты Нико, гоночный инженер. Напиши пост.

СЕГОДНЯ: {current_date}
ВАЖНО: ПИШИ ТОЛЬКО ПРО {current_year} ГОД. НЕ УПОМИНАЙ 2024, 2025.
...
async def rate_post(post: str, title: str) -> bool:
    ...
    if has_old_date(post):
        print(f"❌ Пост отклонен: содержит старый год")
        return False
