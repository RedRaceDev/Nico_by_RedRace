...
def extract_article_sync(url: str) -> dict:
    """Синхронная функция для Newspaper3k"""
    try:
        article = Article(url)
        article.download()
        article.parse()
...
async def fetch_article_fallback(url: str) -> str:
    try:
...
def is_real_news(text: str) -> bool:
    """Проверяет, что текст — реальная новость, а не список тем"""
    trash_markers = [
        "смотрите также", "читайте также", "список десяти лучших",
        "вопросы о применении", "упоминается сюжет", "также затрагиваются",
        "также упоминается", "в статье упоминается", "рейтинг"
    ]
...
def clean_summary(text: str) -> str:
    """Берем только первый нормальный абзац"""
    if not text:
        return ""
    # Берем первый абзац
    first_para = text.split('\n\n')[0] if '\n\n' in text else text
    # Берем первое предложение, если коротко
    if len(first_para) < 100:
        first_para = text.split('.')[0] + '.'
    return first_para[:800]
...
# === ГЕНЕРАЦИЯ ПОСТА ИЗ ПОЛНОЙ СТАТЬИ ===
async def gen_post_from_article(article_data: dict, title: str) -> str:
    article_text = article_data.get("text", "")[:1500]
    article_summary = article_data.get("summary", "")[:500]
    
    content = article_text if len(article_text) > 200 else article_summary
...
async def monitor(callback):
    ...
    while True:
        for src in RSS_SOURCES:
            ...
            # === ВЫТАСКИВАЕМ ПОЛНЫЙ ТЕКСТ ЧЕРЕЗ NEWSPAPER3K ===
            article_data = await fetch_full_article(link)
            
            # Если Newspaper не вытянул — пробуем fallback через RSS summary
            if not article_data.get("text") and not article_data.get("summary"):
                summary = entry.get('summary', '')[:800]
                if not is_real_news(summary):
                    print(f"❌ Пост забракован (не новость): {entry.get('title', '')[:50]}")
                    mark_posted(entry.get('title', ''), link)
                    continue
            else:
                summary = article_data.get("summary", "")[:800]
            
            # Генерируем пост
            post = await gen_post_from_article(article_data, entry.get('title', ''))
            
            # Оцениваем качество
            if await rate_post(post, entry.get('title', '')):
                await callback(post, entry.get('title', ''), link)
            else:
                print(f"❌ Пост забракован (оценка): {entry.get('title', '')[:50]}")
                mark_posted(entry.get('title', ''), link)
