...
def get_admin_keyboard():
    ...
    markup.add(
        ...
        KeyboardButton("📰 Показать новости"), KeyboardButton("✅ Опубликовать всё"),
...
def get_user_keyboard():
    ...
    markup.add(
        ...
        KeyboardButton("🎮 Карточная игра F1")
    )
    return markup
...
# === НОВЫЕ ФУНКЦИИ ДЛЯ РУЧНОГО ПОСТИНГА ===
async def show_pending_posts(m):
    posts = get_pending_posts()
    if not posts:
        await bot.send_message(m.chat.id, "📭 Нет новых новостей для публикации")
        return
    text = f"📰 **Новости для публикации ({len(posts)})**\n\n"
    for i, p in enumerate(posts, 1):
        text += f"{i}. {p['title'][:80]}\n"
    text += "\nНажми ✅ Опубликовать всё"
    await bot.send_message(m.chat.id, text, parse_mode="HTML")

async def publish_all_posts(m):
    posts = get_pending_posts()
    if not posts:
        await bot.send_message(m.chat.id, "📭 Нет новостей для публикации")
        return
...
async def test_mode_cmd(m):
    ...
    await bot.send_message(m.chat.id, "🎮 Тестовый режим включен. Напиши любой запрос.")
...
async def bug_report(m):
    global wait_bug
    await bot.send_message(m.chat.id, "🐞 Опиши баг подробно.")
    wait_bug = True
...
async def save_bug_report(m):
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            pass
    await bot.send_message(m.chat.id, "✅ Баг отправлен админу")
...
async def reset_post_timeout():
    ...
    if wait_post:
        wait_post = False
        print("⚠️ Режим публикации отключен")
...
📰 Новостей на публикацию: {pending}
...
🎮 А ещё у нас есть карточная игра по F1 — @sipmly_flag_bot
...
async def handle_msg(m):
    ...
    if is_admin(m.chat.id) and m.text:
        if m.text == "📜 История диалогов":
            ...
            return
        elif m.text == "📰 Показать новости":
            await show_pending_posts(m)
            return
        elif m.text == "✅ Опубликовать всё":
    ...
    if m.text == "📅 Календарь":
        ...
        await bot.send_message(m.chat.id, get_random_character(), parse_mode="HTML")
        return
    elif m.text == "🎮 Карточная игра F1":
        await bot.send_message(m.chat.id, "🎴 **Карточная игра по Формуле-1**\n\nСобирай пилотов, трассы и создавай коллекцию.\n\n🎲 **Игра:** @sipmly_flag_bot\n\nRed Race | Подписаться", parse_mode="HTML")
        return
...
async def main():
    ...
    print("📰 Мониторинг активен, новости ждут публикации")
