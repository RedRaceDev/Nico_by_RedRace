...
from scraper import (
    ...
    generate_image_agnes, save_to_memory, get_memory
)
...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if len(self.users[user_id]) >= self.limit:
            ...
            return  # вместо CancelHandler просто выходим
...
def get_user_keyboard():
    buttons = [
        ...
        [KeyboardButton(text="🎨 Сгенерировать картинку"), KeyboardButton(text="📰 Новости")],
...
async def cmd_image(message: Message):
    await message.answer("🎨 Напиши промпт для генерации картинки.\n\nПример: Ferrari SF-26 на закате в Монако")
    await Form.waiting_for_image_prompt.set()

@dp.message(Command("news"))
async def cmd_news(message: Message):
    status = await message.answer("🔍 Загружаю свежие новости F1...")
    prompt = "Найди последние новости Формулы-1 за последние 24 часа"
    news = await ask_ai(prompt, need_search=True)
    await status.delete()
...
async def cmd_donate(message: Message):
    await message.answer(
        "⭐ **Поддержать RedRace Development**\n\n"
        "Выбери сумму:",
...
async def cmd_pay_support(message: Message):
    await message.answer("По вопросам возврата: @Scanialove или /help")
...
async def process_donate_choice(callback: CallbackQuery, state: FSMContext):
    amount_str = callback.data.split("_")[1]

    if amount_str == "custom":
        await callback.message.answer("💰 Отправь число звезд (от 1 до 2500):")
...
async def process_custom_donate(message: Message, state: FSMContext):
    await state.clear()
    if not message.text.isdigit() or not 1 <= int(message.text) <= 2500:
        await message.answer("❌ Число должно быть от 1 до 2500")
        return
...
@dp.message(lambda m: m.text == "⭐ Поддержать проект")
...
async def process_image_prompt(message: Message, state: FSMContext):
    ...
    url = await generate_image_agnes(message.text)
    if url:
        await bot.send_photo(message.chat.id, url, caption=f"🎨 По запросу: {message.text}")
        await status.delete()
    else:
        await status.edit_text("❌ Не удалось сгенерировать картинку. Попробуй другой промпт.")
...
async def chat_handler(message: Message):
    ...
    buttons = ["🏆 Чемпионат", "📅 Календарь", "🏁 Следующая гонка", "🎭 Персонаж",
               "🎨 Сгенерировать картинку", "📰 Новости", "⭐ Поддержать проект", "📊 Статистика",
               "👥 Пользователи", "📝 Пост в канал", "✅ Опубликовать всё", "🧹 Очистить БД", "ℹ️ Помощь"]
    if text in buttons:
        return

    status = await message.answer("🤔 Думаю...")
    need_search = any(word in text.lower() for word in ['новости', 'последние', 'сегодня', 'результаты'])
    answer = await ask_ai(text, need_search=need_search)
    if len(answer) > 500:
        answer = answer[:500] + "..."
