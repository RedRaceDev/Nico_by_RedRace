...
from scraper import (
    ...
    save_to_memory, get_memory, get_f1_news, get_f1_news_fallback
)
...
def get_user_keyboard():
    buttons = [
        ...
        [KeyboardButton(text="⭐ Поддержать проект"), KeyboardButton(text="📰 Новости")],
...
async def cmd_news(message: Message):
    status = await message.answer("🔍 Ищу свежие новости F1...")
    
    # Пробуем TinyFish
    news = await get_f1_news(limit=6)
    
    # Если TinyFish не ответил, пробуем RSS
    if not news:
        news = await get_f1_news_fallback(limit=6)
    
    if news:
        await status.delete()
        await message.answer(news, parse_mode="HTML", disable_web_page_preview=True)
    else:
        await status.edit_text("❌ Не удалось загрузить новости. Попробуй позже.")
...
async def chat_handler(message: Message):
    ...
    buttons = ["🏆 Чемпионат", "📅 Календарь", "🏁 Следующая гонка", "🎭 Персонаж",
               "📰 Новости", "⭐ Поддержать проект", "📊 Статистика", "👥 Пользователи", 
               "📝 Пост в канал", "✅ Опубликовать всё", "🧹 Очистить БД", "ℹ️ Помощь", "⭐ Донаты"]
    if text in buttons:
        return
...
async def main():
    ...
    logger.info("🤖 AI: Nex-N2-Pro + Nemotron 3 Ultra via OpenRouter")
    logger.info("🔍 Новости: TinyFish Search API + RSS fallback")
