...
from scraper import (
    ...
    generate_morning_digest, get_interesting_fact
)
...
async def health_check(request):
    return web.Response(text="Nico 6.5 is alive", status=200)
...
async def admin_panel(m):
    ...
    status = (
        ...
        f"└ 👑 Админ: <b>Активен</b>\n"
        f"<code>═══════════════════════════</code>\n"
        f"<i>Nico 6.5 | Code by: RedRace Development</i>"
    )
...
async def user_panel(m):
    ...
    status = (
        ...
        f"<code>Nico 6.5 | RedRace Development</code>"
    )
    ...
    try:
        if any(word in user_text.lower() for word in ["выложи", "сделай пост", "напиши пост"]):
            if not is_admin(m.chat.id):
                ...
                if not topic:
                    topic = "новости F1"
                await bot.send_message(m.chat.id, f"📝 Генерирую пост про {topic}...")
...
async def handle_callbacks(call):
    ...
    if call.data == "model_menu":
        ...
        await bot.send_message(call.message.chat.id, "📨 Генерирую утренний дайджест...")
