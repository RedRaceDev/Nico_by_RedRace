...
from scraper import (
    ...
    monitor_rss, mark_as_posted, get_all_news, clean_post, is_valid_post
)
...
MONITORING_ACTIVE = True  # Глобальная переменная
bot = None
...
# === Запуск ===
async def main():
    global bot, waiting_for_search, waiting_for_topic
    bot = AsyncTeleBot(BOT_TOKEN)
    
    # Устанавливаем флаги для callback'ов
    @bot.callback_query_handler(func=lambda call: True)
    async def callback_handler(call):
        if call.data == "make_post":
            waiting_for_topic = True
            await bot.answer_callback_query(call.id, "Введи тему")
        elif call.data == "search":
            waiting_for_search = True
            await bot.answer_callback_query(call.id, "Введи запрос")
        else:
