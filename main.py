...
from scraper import (
    ...
    mark_posted, morning_digest, ask_nano_omni, ask_gemini,
...
def get_beta_keyboard():
    ...
    markup.add(
        ...
        KeyboardButton("🔬 Режим отладки"),
        KeyboardButton("📈 Телеметрия"),
        KeyboardButton("🎮 Тестовый режим"),
        KeyboardButton("🔐 Бета-консоль"),
...
async def handle_test_mode(m):
    ...
    raw = await ask_nano_omni(m.text)
...
async def user_panel(m):
    status = f"""🏎️ Привет! Я Нико, твой гоночный инженер.
...
async def handle_msg(m):
    ...
    if m.photo:
        ...
        await bot.reply_to(m, "📸 Фото получил! Анализирую...")
        analysis = await ask_nano_omni("Опиши эту машину Формулы-1 или деталь. Что здесь изображено?", photo_url)
        await bot.reply_to(m, f"🔍 **Анализ фото:**\n\n{analysis}", parse_mode="HTML")
        return
...
async def main():
    ...
    print("🔍 Поиск: SearXNG + DuckDuckGo")
    print("🧠 Модель: NVIDIA Nemotron 3 Nano Omni")
