...
# === СТАБИЛЬНАЯ ОТПРАВКА ЧЕРЕЗ ENTITIES ===
def send_telegram_message(chat_id: str, text: str) -> bool:
    """Отправляет сообщение через entities — без ошибок форматирования"""
    try:
        text, entities = parse_markdown_to_entities(text)
...
async def handle_msg(m):
    ...
    if m.chat.id in ADMIN_IDS:
        if m.text == "📝 Пост на тему":
            ...
            send_telegram_message(m.chat.id, post)
            inc_posts()
            return
        elif m.text == "🔍 Поиск":
            wait_search = True
            ...
            send_telegram_message(m.chat.id, cal)
            return
            ...
            send_telegram_message(m.chat.id, info)
            return
...
async def morning_digest_worker():
    while True:
        ...
        try:
            ...
            send_telegram_message(CHANNEL_ID, digest)
...
async def main():
    ...
    print("🧠 Модели: Xiaomi MiMo-V2-Pro, Tencent Hy3, NVIDIA Nemotron 3, DeepSeek V4")
    print("📡 RSS: Autosport, Motorsport, The Race, PlanetF1, Crash, F1News, F1-Gate, Al Jazeera")
    print("🔒 Отправка через entities — без ошибок форматирования")
