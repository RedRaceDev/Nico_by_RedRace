...
from scraper import (
    generate_posts_pack, get_f1_calendar, chat_with_nico, search_web,
    get_top_news, get_weather_for_track, get_quote_of_the_day
)
...
async def handle_menu_buttons(m):
    ...
    if text == "🏁 О боте":
        ...
        status = await bot.send_message(m.chat.id, "🔍 Собираю новости...")
        ...
        status = await bot.send_message(m.chat.id, "🌍 Ищу погоду...")
...
async def manual_trigger(m):
    ...
    if m.text in ["🏁 О боте", "📅 Календарь", "🏆 Топ новостей", "🌦️ Погода", "💬 Чат", "🏎️ Цитата дня",
...
async def start_command(m):
    if is_admin(m.chat.id):
        # Показываем админу консоль и меню
...
async def ask_command(m):
    ...
    result = await search_web(f"F1 {args[1]} 2026", max_results=3)
    await bot.delete_message(m.chat.id, status_msg.message_id)
